...
class Agente_Model_DbTable_TbVinculo extends MinC_Db_Table_Abstract
{
    ...
    public function buscarVinculoProponenteResponsavel($where=array())
    {
        ...
        $slct->joinInner(
                array('ag' => 'agentes'),
            "ag.idagente = vi.idagenteproponente",
                array('ag.cnpjcpf'),
                $this->_schema
        );
        ...
        $slct
            ->joinLeft(
                array('sga' => 'sgcacesso'),
                "sga.idusuario = vi.idusuarioresponsavel",
        ...
        $slct->joinLeft(
                array('vp' => 'tbvinculoproposta'),
            "vp.idvinculo = vi.idvinculo",
    ...
    public function buscarProponenteResponsavel($idUsuarioLogado, $mecanismo = false)
    {
        $slct1 = $this->select()
                    ...
                    ->from(
                            array('a' => 'Projetos'),
                            array(new Zend_Db_Expr('0 as Ordem'),'CgcCpf as CNPJCPF'),
                        $this->getSchema('sac')
                    )
                    ->joinInner(
                            array('b' => 'Agentes'),
        ...
        $slct2 = $this->select()
                    ...
                    ->from(
                            array('a' => 'Projetos'),
                            array(new Zend_Db_Expr('1 as Ordem')),
                        $this->getSchema('sac')
                    )
                    ->joinInner(
                            array('b' => 'Agentes'),
                        "a.CgcCpf = b.CNPJCPF",
                            array('CNPJCPF', 'idAgente', new Zend_Db_Expr('sac.dbo.fnNome(b.idAgente) as NomeProponente')),
                        $this->getSchema('agentes')
                    )
                    ->joinInner(
        ...
        $slct3 = $this->select()
                    ...
                    ->from(
                            array('a' => 'Projetos'),
                            array(new Zend_Db_Expr('2 as Ordem')),
                        $this->getSchema('sac')
                    )
                    ->joinInner(
                            array('b' => 'Agentes'),
    ...
    public function buscarResponsaveis($where=array(), $idAgenteProponente)
    {
        ...
        $slct->from(
                array('SGA' => 'SGCacesso'),
                array('SGA.Nome AS nomeresponsavel', 'SGA.Cpf AS cpfresponsavel', 'SGA.IdUsuario AS idresponsavel'),
            $this->getSchema('controledeacesso')
        );

        $slct->joinLeft(
                array('VI' => $this->_name),
            'VI.idUsuarioResponsavel = SGA.IdUsuario AND VI.idAgenteProponente = '.$idAgenteProponente,
                $this->_getCols(),
            $this->getSchema('agentes')
        );
    ...
    public function buscarProponentes($idResponsavel)
    {
        $slctResponsavel = $this->select()
                                ...
                                ->from(
                                        array('a' => 'Agentes'),
                                        array('CNPJCPF', 'idAgente', new Zend_Db_Expr('sac.dbo.fnNome(a.idAgente) AS NomeProponente')),
                                    $this->getSchema('agentes')
                                )
                                ->joinInner(
                                        array('c' => 'SGCacesso'),
        ...
        $slctProponentes = $this->select()
                                ...
                                ->from(
                                        array('a' => 'Agentes'),
                                        array('CNPJCPF', 'idAgente', new Zend_Db_Expr('sac.dbo.fnNome(idAgente) as NomeProponente')),
                                    $this->getSchema('agentes')
                                )
                                ->joinInner(
                                        array('v' => 'tbVinculo'),
                                    "a.idAgente = v.idAgenteProponente",
                                        array(),
                                    $this->getSchema('agentes')
                                )
                                ->joinInner(
                                        array('k' => 'SGCacesso'),
                                    "k.IdUsuario = v.idUsuarioResponsavel",
                                        array(),
                                    $this->getSchema('controledeacesso')
                                )
