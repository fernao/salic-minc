...
class Autenticacao_Model_DbTable_Usuario extends MinC_Db_Table_Abstract
{
    ...
    /**
     * Novo metodo para alterar a senha do usuario do sistema (Solicitacao CGTI - Dia 15/01/2015)
     * @access public
     * @param @cpf (cpf ou cnpj do usuario)
    ...
    public function salvarDados($dados)
    {
        ...
        $tmpTblUsuario = new Autenticacao_Model_DbTable_Usuario();
    ...
    public function salvarNovoUsuario($cpf, $nome, $idUnidade, $idUsuarioLogado, $idOrgaoUsuarioLogado)
    {
        try {
            ...
            $usuarioCadastrado = $usuarios->buscar(array('usu_identificacao = ?' => $cpf))->current();

            if (!empty($usuarioCadastrado)) {
                throw new Exception("CPF j&aacute; cadastrado!");
            }

            $pessoa = new Pessoas();
            $ultimoCadastroPessoa = $pessoa->buscar(array(), array('pes_codigo desc'), array(1))->current();
            $idPessoa = $ultimoCadastroPessoa->pes_codigo + 1;
            ...
            $dados = array(
                ...
                "pes_validade" => 0,
                "pes_orgao_cadastrador" => $idOrgaoUsuarioLogado,
                "pes_usuario_cadastrador" => $idUsuarioLogado,
            ...
            $dadosIdentificacao = array(
                ...
                "pid_meta_dado" => 1,
                "pid_sequencia" => 1,
                "pid_identificacao" => $idUnidade
            );

            $pessoaIdentificacao = new Pessoaidentificacoes();
            $pessoaIdentificacao->salvarDados($dadosIdentificacao);
